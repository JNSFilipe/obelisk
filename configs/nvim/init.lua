...
vim.pack.add({
  ...
  {src = compmod_src, name = "compmod.nvim"},  -- { src = "https://github.com/mfussenegger/nvim-dap" },
...
local ok_minifiles, minifiles = pcall(require, "mini.files")
if ok_minifiles then
	minifiles.setup({
		windows = {
			preview = true,
			width_preview = 80,
...
local ok_minisurround, minisurround = pcall(require, "mini.surround")
if ok_minisurround then
	minisurround.setup()
end

local ok_which_key, which_key = pcall(require, "which-key")if ok_which_key then
  which_key.setup({
    icons = { group = "" },
  })
  which_key.add({
...
api.nvim_create_autocmd("TextYankPost", {
  ...
  group = api.nvim_create_augroup("snacks-alt-autoread", { clear = true }),
  callback = function()
  ...
  group = api.nvim_create_augroup("snacks-alt-autoread-notify", { clear = true }),
  callback = function()
  	vim.notify("File changed on disk. Buffer reloaded.", vim.log.levels.INFO)
  end,
  })

  map("v", "J", ":m '>+1<CR>gv=gv", "Move selection down")map("v", "K", ":m '<-2<CR>gv=gv", "Move selection up")
...
map({ "i", "n" }, "<C-x><C-f>", function()
	if ok_minifiles then
		if not minifiles.close() then
			minifiles.open(api.nvim_buf_get_name(0), true)
		end
	else
		notify_missing("mini.files", "package not loaded")
	end
end, "Mini Files")
...
map("n", "<leader>u", vim.cmd.UndotreeToggle, "Undo history")
...
map("n", "<leader>o", function()
	if ok_oil then
		...
		notify_missing("oil.nvim", "package not loaded")
	end
end, "Explorer")
